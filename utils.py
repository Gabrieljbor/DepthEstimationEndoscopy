...
def load_images(image_files):
    ...
    for file in image_files:
        ...
        x = np.clip(np.asarray(rgb_image, dtype=float) / 255, 0, 1)
        loaded_images.append(x)
    return np.stack(loaded_images, axis=0)

def load_inputs(input_file_names):
    inputs = []
    with open(input_file_names, "r") as file:
        for line in file:
            inputFileName = line.strip()
            inputFileName = inputFileName.replace("rgb", "examples")
